...
@Service
public class ConferenceProceedingsExcelExportService {
    ...
    public ByteArrayInputStream exportToExcel(List<ConferenceProceedings> proceedingsList) {
        try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            Sheet sheet = workbook.createSheet("DHET Proceedings Report");
            createHeader(sheet, workbook);

            List<ConferenceProceedings> readyForPosting = proceedingsList == null ? List.of() : proceedingsList.stream()
                    .filter(item -> item != null && item.getStatus() == ProceedingsStatus.READY_FOR_POSTING)
                    .toList();

            int rowIdx = 1;
            for (ConferenceProceedings proceedings : readyForPosting) {
                List<Author> affiliatedAuthors = getAffiliatedAuthors(proceedings);
                if (affiliatedAuthors.isEmpty()) {
                    rowIdx = writeRow(sheet, rowIdx, proceedings, null);
                } else {
                    for (Author author : affiliatedAuthors) {
                        rowIdx = writeRow(sheet, rowIdx, proceedings, author);
                    }
                }
            }
            ...
            throw new RuntimeException("Failed to export conference proceedings report to Excel", e);
        }
    }
    ...
    private int writeRow(Sheet sheet, int rowIndex, ConferenceProceedings proceedings, Author author) {
        ...
        List<String> values = List.of(
                str(proceedings.getDhetNo()),
                str(proceedings.getOriginalOrPhotocopy()),
                str(proceedings.getEvidenceOfPeerReview()),
                str(proceedings.getTypeOfEvidence()),
                str(proceedings.getYearOfPublication()),
                str(proceedings.getTitleOfProceeding()),
                str(proceedings.getTitleOfContribution()),
                str(proceedings.getCompliesWith60Rule()),
                str(proceedings.getEditors()),
                str(proceedings.getPublisher()),
                str(resolveIsbnIssn(proceedings)),
                str(proceedings.getFieldOfResearch()),
                str(proceedings.getStartDate()),
                str(proceedings.getEndDate()),
                str(proceedings.getCity()),
                str(proceedings.getCountry()),
                str(proceedings.getFunders()),
                ...
                num(proceedings.getUnits() == null ? null : proceedings.getUnits().getMaxUnitsForPublication()),
                num(proceedings.getUnits() == null ? null : proceedings.getUnits().getTotalProportionOfAuthors()),
                str(proceedings.getUnits() == null ? null : proceedings.getUnits().getAuthorCount()),
                num(proceedings.getUnits() == null ? null : proceedings.getUnits().getTotalUnitsClaimed()),
                nonAffiliatedAuthorsSummary(proceedings),
                str(proceedings.getAdditionalProceedingComments()),
                "",
                "",
                ""
        );
    ...
    private List<Author> getAffiliatedAuthors(ConferenceProceedings proceedings) {
        if (proceedings == null || proceedings.getAuthors() == null) {
            return List.of();
        }
        return proceedings.getAuthors().stream()
                .filter(author -> author != null && Boolean.TRUE.equals(author.getAffiliation()))
                .toList();
    }

    private String nonAffiliatedAuthorsSummary(ConferenceProceedings proceedings) {
        if (proceedings == null || proceedings.getAuthors() == null) {
            return "";
        }
        return proceedings.getAuthors().stream()
    ...
    private String resolveIsbnIssn(ConferenceProceedings proceedings) {
        String issn = str(proceedings.getIssn()).trim();
        if (!issn.isBlank()) {
            return issn;
        }
        return str(proceedings.getIsbn());
    }
