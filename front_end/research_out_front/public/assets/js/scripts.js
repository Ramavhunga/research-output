!(function (NioApp, $) {
    ...
    // Tab Scrolling @v8.0
    NioApp.TabScroll = function (elm, opt) {
        const selector = elm || '.nk-menu-wrap';
        
        if ($(selector).exists()) {
        $(selector).each(function () {
            const $tabsContainer = $(this);
            // Find the actual tabs container within the wrapper
            const $tabsWrapper = $tabsContainer.find('.nk-tab-scroll').first();
            
            if (!$tabsWrapper.length) return; // Skip if no inner tab elements found
            
            // Configuration
            const config = {
            scrollSpeed: $tabsContainer.data('scroll-speed') || 150,
            showButtons: $tabsContainer.data('show-buttons') !== false,
            showGradients: $tabsContainer.data('show-gradients') !== false,
            autoHideButtons: $tabsContainer.data('auto-hide-buttons') !== false
            };
            
            // Extend with options if provided
            const settings = opt ? $.extend(config, opt) : config;
            
            // Setup tab container for scrolling
            function setupTabContainer() {
                // Add positioning to container (don't modify existing tab classes)
                $tabsContainer.addClass('tabs-scrollable');
                
                // Only set scroll-related styling for the tabs wrapper
                $tabsWrapper.css({
                    'overflow-x': 'scroll',
                ...
                $tabsWrapper.css('-webkit-overflow-scrolling', 'touch');
                
                // Prevent tab wrapping
                $tabsWrapper.find('> li, > div, > a, > button').css({
                    'flex-shrink': '0'
                });
                
                // Add scroll buttons if enabled
                if (settings.showButtons) {
                    createScrollButtons();
                }
                
                // Add gradient indicators if enabled
                ...
                // Listen for resize events to update buttons visibility
                $(window).on('resize', function() {
                    updateScrollButtonsVisibility();
                });
                
                // Handle mousewheel scrolling
                $tabsWrapper.on('mousewheel DOMMouseScroll', function(e) {
                ...
                const styleId = 'nk-tab-scroll-style-' + Math.floor(Math.random() * 1000);
                if (!document.getElementById(styleId)) {
                    $('<style id="' + styleId + '">' + 
                    selector + ' .nk-menu::-webkit-scrollbar, ' + 
                    selector + ' .nav-tabs::-webkit-scrollbar, ' + 
                    selector + ' .tab-nav::-webkit-scrollbar, ' + 
                    selector + ' .nav-pills::-webkit-scrollbar { display: none; }' + 
                    '</style>').appendTo('head');
                }
            }
            ...
            $tabsContainer.find('.nk-tab-scroll-btn').remove();
            
            // Use NioApp icons for consistency
            const $leftButton = $('<button class="nk-tab-scroll-btn nk-tab-scroll-btn-left" aria-label="Scroll tabs left">' +
                '<em class="icon ni ni-chevron-left"></em></button>');
            const $rightButton = $('<button class="nk-tab-scroll-btn nk-tab-scroll-btn-right" aria-label="Scroll tabs right">' +
                '<em class="icon ni ni-chevron-right"></em></button>');
            
            // Style the buttons - using NioApp's design patterns
            const btnStyle = {
                'position': 'absolute',
                'top': '50%',
                'transform': 'translateY(-50%)',
                'z-index': '2',
                'background-color': 'var(--e-pglum-orange)',
                'color': '#fff',
            ...
            $leftButton.css(btnStyle).css({
                'left': '0'
            });
            
            $rightButton.css(btnStyle).css({
                'right': '0'
            });
            
            // Append to container
            $tabsContainer.append($leftButton);
            $tabsContainer.append($rightButton);
            
            // Add event listeners
            $leftButton.on('click', function() {
                $tabsWrapper.animate({
                scrollLeft: $tabsWrapper.scrollLeft() - settings.scrollSpeed
                }, 300);
            });
            ...
            $leftButton.hover(
                function() {
                $(this).css({
                    'background-color': 'rgba(241, 175, 96, 0.9)',
            ...
            $rightButton.hover(
                function() {
                $(this).css({
                    'background-color': 'rgba(241, 175, 96, 1)',
                    'box-shadow': '0 3px 12px rgba(43, 55, 72, 0.15)'
                });
                },
                function() {
            ...
            // Create gradient indicators for scroll
            function createGradientIndicators() {
            // Remove if already exist
            $tabsContainer.find('.nk-tab-scroll-gradient').remove();
            
            // Create gradient indicators
            const $leftGradient = $('<div class="nk-tab-scroll-gradient nk-tab-scroll-gradient-left"></div>');
            const $rightGradient = $('<div class="nk-tab-scroll-gradient nk-tab-scroll-gradient-right"></div>');
            ...
            $leftGradient.css(gradientStyle).css({
                'left': '0',
                'background': 'linear-gradient(to right, rgba(255,255,255,0.9), rgba(255,255,255,0))'
            });
            
            $rightGradient.css(gradientStyle).css({
                'right': '0',
                'background': 'linear-gradient(to left, rgba(255,255,255,0.9), rgba(255,255,255,0))'
            });
            
            // Append to container
            $tabsContainer.append($leftGradient);
            $tabsContainer.append($rightGradient);
            ...
            const maxScrollLeft = $tabsWrapper[0].scrollWidth - $tabsWrapper[0].clientWidth;
            
            if (scrollLeft > 10) {
                $tabsContainer.find('.nk-tab-scroll-gradient-left').css('opacity', '1');
            } else {
                $tabsContainer.find('.nk-tab-scroll-gradient-left').css('opacity', '0');
            }
            
            if (scrollLeft < maxScrollLeft - 10) {
                $tabsContainer.find('.nk-tab-scroll-gradient-right').css('opacity', '1');
            } else {
                $tabsContainer.find('.nk-tab-scroll-gradient-right').css('opacity', '0');
            }
            }
            ...
            const isScrollable = $tabsWrapper[0].scrollWidth > $tabsWrapper[0].clientWidth;
            
            if (!isScrollable) {
                // Hide all indicators if content fits
                $tabsContainer.find('.nk-tab-scroll-btn, .nk-tab-scroll-gradient').css({
                'opacity': '0',
                'visibility': 'hidden'
                });
                $tabsContainer.removeClass('has-scroll');
                return;
            } else {
                $tabsContainer.addClass('has-scroll');
            }
            ...
            // Update button visibility if auto-hide is enabled
            if (settings.showButtons && settings.autoHideButtons) {
                const scrollLeft = $tabsWrapper.scrollLeft();
                const maxScrollLeft = $tabsWrapper[0].scrollWidth - $tabsWrapper[0].clientWidth;
                
                if (scrollLeft <= 10) {
                $tabsContainer.find('.nk-tab-scroll-btn-left').css({
                    'opacity': '0',
                    'visibility': 'hidden'
                });
                } else {
                ...
                if (scrollLeft >= maxScrollLeft - 10) {
                $tabsContainer.find('.nk-tab-scroll-btn-right').css({
                    'opacity': '0',
                    'visibility': 'hidden'
                });
                } else {
                ...
                // Always show buttons if auto-hide is disabled
                $tabsContainer.find('.nk-tab-scroll-btn').css({
                'opacity': '1',
                'visibility': 'visible'
                });
            }
            ...
            // Initialize the tab scrolling
            setupTabContainer();
            
            // Scroll to active tab on init if it exists
            const $activeTab = $tabsWrapper.find('.active, .current-page').first();
            if ($activeTab.length) {
            setTimeout(function() {
                const tabPos = $activeTab.position().left;
                const containerWidth = $tabsWrapper.width();
                const tabWidth = $activeTab.width();
                
                // Center the active tab
                $tabsWrapper.scrollLeft(tabPos - (containerWidth / 2) + (tabWidth / 2));
            }, 100); // Short delay to ensure elements are fully rendered
            }
        });
        }
    };
    ...
    // TabScroll Init @v8.0
    NioApp.TabScroll.init = function () {
        // Initialize for main wrapper that contains tabs
        NioApp.TabScroll('.nk-menu-wrap');
        
        // Also check for any manually added tab containers
        NioApp.TabScroll('.tabs-scrollable');
    };
    ...
    // Profile Background Adjust @v1.0
    NioApp.ProfileBgAdjust = function () {
        function adjustProfileBackground() {
            const header = document.querySelector('.nk-header');
            const profileDropdown = document.querySelector('.nk-header-tools');

            if (header && profileDropdown) {
                const profileWidth = profileDropdown.offsetWidth;
                const finalWidth = profileWidth + 32;
                header.style.setProperty('--profile-width', `${finalWidth}px`);
            }
        }

        // Initial adjustment
        adjustProfileBackground();

        // Adjust on window resize
        $win.on('resize', adjustProfileBackground);
    };

    // Extend NioApp with HTMX integration functions
    NioApp.HtmxIntegration = function() {
        // Listen for HTMX events
        document.body.addEventListener('htmx:afterSwap', function(event) {
            // Check if the target matches our owner details container
            if(event.detail.target.querySelector('#dataTable_swap')) {
                // Refresh/reinitialize the DataTable
                if($.fn.DataTable.isDataTable('#dataTable_swap')) {
                    // If table already exists, refresh it
                    $('#dataTable_swap').DataTable().ajax.reload();
                } else {
                    // Otherwise initialize
                    NioApp.DataTable('#dataTable_swap', {
                        responsive: {
                            details: true
                        }
                    });
    ...
    NioApp.init = function () {
        ...
        NioApp.coms.docReady.push(NioApp.HtmxIntegration);
    }
